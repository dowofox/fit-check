...
test("세분화된 오픈형 신발과 레인부츠의 계절 용도를 구분한다", () => {
  const slides = inferSeasonsFromOfficialProduct({
    productName: "MINIMAL SLIDES",
    productCategory: "Fashion > Shoes",
  });
  const clogs = inferSeasonsFromOfficialProduct({
    productName: "LIGHTWEIGHT CLOG",
    productCategory: "Fashion > Shoes",
  });
  const rainBoots = inferSeasonsFromOfficialProduct({
    productName: "CLASSIC RAIN BOOTS",
    productCategory: "Fashion > Shoes > Boots",
  });

  assert.deepEqual(slides.seasons, ["여름"]);
  assert.deepEqual(clogs.seasons, ["여름"]);
  assert.deepEqual(rainBoots.seasons, ["봄", "여름", "가을"]);
  assert.equal(rainBoots.needsReview, false);
});

test("계절성이 명확한 공식 상품 카테고리만 계절 근거로 사용한다", () => {
  const rashGuard = getConfirmedProductSeasonInference({
    brand: "NAES",
    productName: "ESSENTIAL 10",
    productCategory: "Apparel > Swimwear > Rash Guards",
    confirmedAt: "2026-07-15T00:00:00.000Z",
  });
  const raincoat = getConfirmedProductSeasonInference({
    brand: "NAES",
    productName: "ESSENTIAL 11",
    productCategory: "Apparel > Outerwear > Raincoats",
    confirmedAt: "2026-07-15T00:00:00.000Z",
  });
  const skiJacket = getConfirmedProductSeasonInference({
    brand: "NAES",
    productName: "ESSENTIAL 12",
    productCategory: "Apparel > Sportswear > Ski Jackets",
    confirmedAt: "2026-07-15T00:00:00.000Z",
  });
  const genericJacket = getConfirmedProductSeasonInference({
    brand: "NAES",
    productName: "ESSENTIAL 13",
    productCategory: "Apparel > Outerwear > Jackets",
    confirmedAt: "2026-07-15T00:00:00.000Z",
  });

  assert.deepEqual(rashGuard.seasons, ["여름"]);
  assert.deepEqual(raincoat.seasons, ["봄", "가을"]);
  assert.deepEqual(skiJacket.seasons, ["겨울"]);
  assert.equal(genericJacket, null);
});

test("공식 상품명과 카테고리의 계절 근거가 충돌하면 확인 대상으로 남긴다", () => {
  const conflictingProduct = inferSeasonsFromOfficialProduct({
    productName: "반팔 니트",
    productCategory: "Apparel > Outerwear > Coats",
  });
  const specificAllSeasonProduct = inferSeasonsFromOfficialProduct({
    productName: "올시즌 구스 다운 패딩",
    productCategory: "Apparel > Outerwear > Down Jackets",
  });

  assert.deepEqual(conflictingProduct.seasons, ["봄", "여름"]);
  assert.equal(conflictingProduct.needsReview, true);
  assert.match(conflictingProduct.reasons[0], /상품명과 카테고리/);
  assert.deepEqual(specificAllSeasonProduct.seasons, ["겨울"]);
  assert.equal(specificAllSeasonProduct.needsReview, false);
});

test("공식 계절 근거는 충돌하는 기존 사진 분류보다 우선한다", () => {
  const officialCoat = inferSeasonsFromOfficialProduct({
    productName: "ESSENTIAL 04",
    productCategory: "Apparel > Outerwear > Coats",
    currentItem: {
  ...
  const fallbackShortSleeve = inferSeasonsFromOfficialProduct({
    productName: "ESSENTIAL 05",
    currentItem: {
      detailCategory: "반팔 티셔츠",
      subCategory: "티셔츠",
    },
  ...
  assert.deepEqual(officialCoat.seasons, ["가을", "겨울"]);
  assert.equal(officialCoat.needsReview, true);
  assert.match(officialCoat.reasons[0], /기존 사진 분류/);
  assert.deepEqual(fallbackShortSleeve.seasons, ["여름"]);
  assert.equal(fallbackShortSleeve.needsReview, false);
});
