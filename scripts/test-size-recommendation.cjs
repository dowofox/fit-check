...
test("실측 비교의 누락 항목은 현재 의류 카테고리 필드만 포함한다", () => {
  ...
  const upperComparison = getMeasurementComparison(upper, {
    height: "175",
    shoulderWidth: "47",
    chestCircumference: "100",
    armLength: "60",
  });
  const bottomComparison = getMeasurementComparison(bottom, {
    height: "175",
    waistCircumference: "82",
    hipCircumference: "104",
    thighCircumference: "64",
  ...
  assert.deepEqual(upperComparison.unavailableFields, []);
  assert.deepEqual(bottomComparison.unavailableFields, []);
});
...
test("밑위 없는 하의는 인심만으로 총장 추천을 추측하지 않는다", () => {
  const item = createItem("bottom-without-rise", "하의", [
    { size: "M", totalLength: 102, waist: 40, hip: 51, thigh: 32 },
    { size: "L", totalLength: 105, waist: 42, hip: 53, thigh: 34 },
  ]);
  const profile = {
    waistCircumference: "82",
  ...
  assert.equal(result.blockedReason, "missing_profile_measurements");
  assert.deepEqual(result.missingFields, ["키"]);
  assert.deepEqual(result.sizeRecommendations, []);
});

test("모든 하의 후보에 밑위가 있으면 인심으로 총장을 비교한다", () => {
  const item = createItem("bottom-with-rise", "하의", [
    { size: "M", totalLength: 102, waist: 40, hip: 51, thigh: 32, rise: 24 },
    { size: "L", totalLength: 105, waist: 42, hip: 53, thigh: 34, rise: 27 },
  ]);
  const profile = {
    waistCircumference: "82",
    hipCircumference: "102",
  ...
  assert.equal(result.blockedReason, undefined);
  assert.equal(result.recommendedSize, "L");
  assert.match(result.sizeRecommendations[0].reasons.join(" "), /내 인심/);
});
