...
운영 홈은 이 구조를 기준으로 실제 옷장·날씨·저장 코디 데이터를 연결한다.
현재 주간 맥락 안에서 추천을 한 장씩 비교하고, 선택한 코디는 기존 추천
상세 화면으로 이어져 전체 아이템과 이유를 확인한 뒤 저장할 수 있다.
...
## Third Round: Different Entry Models

F의 단계 안내가 여전히 일반적인 앱 대시보드처럼 느껴져, 세부 스타일이
아니라 첫 화면에 들어가는 방식 자체를 바꾼 네 가지 안을 추가했다.

### I. Dual Canvas

첫 화면을 `오늘 코디`와 `새 상품 핏` 두 개의 큰 면으로 나눈다. 메뉴,
현황, 단계 설명을 없애고 사용자가 원하는 목적을 바로 누르게 한다.

- 강점: 가장 즉각적인 이해, 강한 패션 이미지, 선택지가 적음
- 위험: 옷장 현황과 저장 코디 같은 보조 정보의 발견성이 낮음
- 적합 사용자: 앱을 가끔 열고 한 가지 목적만 빠르게 수행하는 사용자

### J. Wardrobe Stage

내 옷 세 벌을 화면 중앙의 무대처럼 먼저 보여주고, 그 아래에서 코디 찾기와
새 상품 핏 확인을 선택한다. 앱 메뉴보다 실제 옷장이 먼저 보이는 구조다.

- 강점: 내 데이터가 있다는 인상, 직관적인 옷장 기반 서비스 표현
- 위험: 옷 사진 품질이 낮거나 옷장이 비어 있을 때 대체 화면이 중요함
- 적합 사용자: 자신의 옷을 보며 조합을 고르는 시각 중심 사용자

### K. Daily Brief

홈을 대시보드가 아니라 오늘의 추천 결과 한 장으로 시작한다. 추천을 먼저
보여주고 `다른 코디`와 `새 옷 핏 보기`를 보조 행동으로 둔다.

- 강점: 재방문 사용자가 가장 빠르게 결과를 확인함
- 위험: 옷이 부족해 추천할 수 없는 첫 사용자에게 별도 진입 구조 필요
- 적합 사용자: 매일 앱을 열어 오늘 입을 옷을 확인하는 사용자

### L. Style Concierge

“오늘은 무엇을 도와드릴까요?”라는 질문 아래 목적을 문장형 목록으로
제시한다. 아이콘이나 탭 이름을 해석하지 않아도 원하는 행동을 고를 수 있다.

- 강점: 초보자 이해도, 설명과 행동의 균형, 작은 화면 안정성
- 위험: 이미지 비중이 적어 패션 브랜드 인상이 약해질 수 있음
- 적합 사용자: 기능을 처음 접하거나 메뉴 구조에 익숙하지 않은 사용자

| 기준 | I Dual Canvas | J Wardrobe Stage | K Daily Brief | L Style Concierge |
| --- | --- | --- | --- | --- |
| 첫 행동 발견 | 가장 빠름 | 빠름 | 결과가 먼저 | 매우 빠름 |
| 패션 이미지 | 매우 강함 | 강함 | 강함 | 중간 |
| 초보 친화성 | 높음 | 높음 | 중간 | 가장 높음 |
| 재방문 효율 | 높음 | 높음 | 가장 높음 | 높음 |
| 빈 옷장 대응 | 별도 화면 필요 | 대체 무대 필요 | 별도 화면 필요 | 가장 자연스러움 |
| 구조 특징 | 2분할 | 옷 중심 무대 | 오늘의 결과 | 질문형 목록 |
...
F. Guided Flow는 1차 운영 적용으로 검토했지만 최종 방향으로 확정하지
않았다. 현재는 I–L 네 가지 진입 모델을 프리뷰에서 비교한 뒤 운영 화면의
새 기준을 다시 선택하는 단계다.
