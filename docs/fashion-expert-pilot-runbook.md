...
각 evaluator output provenance의 `updatedAt`은 그 파일에 든 absolute/pairwise 평가의 `createdAt`과 같거나 이후여야 한다. 병합 시각은 모든 output `updatedAt`과 평가 `createdAt` 이후여야 하며, 순서가 어긋난 병합은 output을 쓰기 전에 거부한다.
