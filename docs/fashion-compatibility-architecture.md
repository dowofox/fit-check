...
- `expert-pilot-batch-lock-v3`는 snapshot과 이미지 바이트뿐 아니라 실제 rubric, anchor, requirement, evidence, rating/availability 계약과 evaluator-visible 선택지 순서·문구의 canonical digest를 전체 batch fingerprint에 포함한다. Registry 또는 presentation contract가 달라지면 rubric version 문자열이 같아도 runner가 listen 전에 거부한다.
- Protocol payload는 실제 registry와 공유 presentation contract에서 생성하며 평가에 영향이 없는 `reviewedBy`와 `sourceReferences`만 제외한다. Dimension/evidence 표시 순서는 semantic array로 보존한다. Lock에는 전체 문구 대신 SHA-256 digest만 저장하고 output sidecar는 batch fingerprint를 통해 protocol을 간접적으로 고정한다.
...
각 평가자 output은 frozen source dataset을 기준으로 해당 evaluator의 absolute evaluation만 변경할 수 있다. 병합 경계에서 v3 batch fingerprint와 output provenance를 검증하고, 병합 시각이 모든 evaluator output 완료 시각과 absolute/pairwise 평가 레코드 생성 시각 이후인지 확인하며, source snapshot·pairwise·split·metadata 계약의 semantic equality를 확인한다. 완료된 evaluator별 delta만 evaluator/outfit/evaluation ID 순으로 합쳐 입력 순서와 무관한 분석용 dataset을 만든다.
